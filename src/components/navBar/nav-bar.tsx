...
export const NavBar = ({ open = false, setOpen, breadcrumb }: Props) => {
  ...
  return (
    <div className="p-4 xl:ml-80">
      <nav
        aria-label="nav"
        className="block w-full max-w-full bg-transparent text-primary_2 shadow-none transition-all px-0 py-1">
        <div className="flex flex-col-reverse justify-between gap-6 md:flex-row md:items-center">
          <div className="capitalize max-sm:mt-14">
              ...
              <ol className="flex flex-wrap items-center w-full bg-opacity-60 rounded-md bg-transparent p-0 transition-all">
                {breadcrumb.map((menu, index) => (
                  <li
                    ...
                    <a href="[menu.link]">
                      <p className="block antialiased font-sans text-sm leading-normal font-normal transition-all hover:text-blue-500 hover:opacity-100">
                        {t(`menu.${menu.title}`)}
                      </p>
                    </a>
            ...
            <h6 className="block antialiased tracking-normal font-sans text-base font-semibold leading-relaxed text-primary_2">
              {t(`menu.${breadcrumb[breadcrumb.length - 1].title}`)}
            </h6>
          </div>
          <div className="flex items-center justify-between max-sm:bg-gradient-to-r max-sm:from-primary_2 max-sm:to-complementary_4 max-sm:rounded-md max-sm:fixed max-sm:w-[calc(100vw_-_30px)] max-sm:top-3 max-sm:z-20">
            <button
