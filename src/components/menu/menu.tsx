...
export const Menu = ({ open = false, setOpen }: Props) => {
  ...
  return (
    <>
      <div
        className={`${open ? '' : 'hidden'} z-40 absolute bg-black opacity-40 top-0 left-0 w-screen h-screen xl:hidden`}></div>

      <aside
        className={`${
          open ? '' : 'hidden'
        } bg-gradient-to-br from-slate-900 to-[#111827] fixed inset-0 z-50 my-4 ml-4 h-[calc(100vh-32px)] rounded-xl transition-transform duration-300 translate-x-0 xl:block md:w-72 md:translate-x-[calc(100vw_-_311px)] xl:translate-x-0 max-md:w-[calc(100%_-_30px)]`}>
